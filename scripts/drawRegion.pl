...
### draw te on ref axis
if (defined $opt{reftegff}){
my $reftegff=parseTEGFF($opt{reftegff});
$svg=drawXTE($svg,$reftegff,$rate,$xx1,$yy1);
}
...
###
if ($opt{type} =~/ACT/){
   drawlinkACT($opt{compare},$rate);
}else{
   drawlinkm8($opt{compare},$rate);
}
...
close OUT;
       #system "/home/jfchen/159/FFproject/tools/draw/svg2xxx_release/svg2xxx $file -t pdf -m 700";
}
